  ...
  <div class="wrapper">
    <header class="text-center">
      ...
      <h2 class="text-2xl my-8">
        A free to use, open source, collection of ({{ snippets.length }})
        <br />Javascript Snippets
      </h2>
    </header>

    <main>
      <section class="text-center bg-white rounded-lg w-2/3 mx-auto p-6">
        <label for="search" class="text-xl">Search all snippets:</label>
        <br />
        <input
          type="text"
          id="search"
          v-model="search"
          class="border rounded p-4 w-2/3 my-4 text-lg"
        />
        <p class="text-xl pb-4">Or, filter by keyword:</p>
        <ul class="flex justify-center space-x-6 text-lg">
          <li
            @click="search = 'array'"
            class="cursor-pointer border rounded-lg px-4 py-2"
          >
            array
          </li>
          <li
  ...
  computed: {
    filteredList() {
      ...
      return this.snippets.filter((snippet) =>
        // or convert tags and title to a complete string and search??
        ...
        snippet.title
          .toLowerCase()
          .split(' ')
          .some((value) => this.search.toLowerCase().split(' ').includes(value))
      )
    },
  },
